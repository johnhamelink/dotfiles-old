...
then
	...
	# Finally, run dzen2
	(uname -a; echo ""; echo "Uptime:" $(uptime | sed 's/.*up//g' | sed 's/,.*//g' | sed 's/\s//g'); sleep 60) | dzen2 -fn '-*-tamsyn-*-*-*-*-12-*-*-*-*-*-iso10646-*' -x $(($CURRENTSCREEN-$1)) -y 15 -h 15 -w 300 -l 1 -sa c -e 'onstart=uncollapse;button1=exit;button3=exit'
fi
